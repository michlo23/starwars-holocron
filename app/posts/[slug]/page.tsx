...
async function getPost(slug: string): Promise<Post | null> {
  const posts = await sql<Post[]>`SELECT * FROM posts WHERE slug = ${slug} LIMIT 1`;
  return posts[0] || null;
}

async function getRelatedPosts(category: string, currentSlug: string): Promise<Post[]> {
  return await sql<Post[]>`SELECT * FROM posts WHERE category = ${category} AND slug != ${currentSlug} ORDER BY RANDOM() LIMIT 3`;
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    ...
    openGraph: { title: post.title_en, description: post.excerpt_en, images: [{ url: post.image_url }] },
  };
}
  ...
  return (
    <div className="min-h-screen bg-[#111114]">
      <div className="relative h-[40vh] min-h-[250px] md:h-[50vh] md:min-h-[400px] overflow-hidden">
        <Image src={post.image_url} alt={t(post, 'title', lang)} fill className="object-cover" priority />
      ...
      {relatedPosts.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 md:px-8 py-16">
          <h2 className="text-2xl font-semibold text-[#fafafa] mb-8 tracking-tight">
            {i18n('post.relatedArchives', lang)}
          </h2>
